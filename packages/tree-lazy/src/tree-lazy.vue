  ...
  watch: {
    ...
    value: {
      async handler(newValue, oldValue) {
        ...
        /*
          以观察value为主,
          异步的原因是当value和data都发生改变的时候,
          等待data赋值完毕再进行后面的同步选项操作 */
          if (unWatchData)
            unWatchData ()
          unWatchData = this.$watch ('data', () => {
  ...
  methods: {
    ...
    syncValueSelect() {
      ...
      if (
        ...
        (store.root.childNodes && !store.root.childNodes.length)
      )
        return store.setCurrentNodeKey();
      function findSelectNode(NodeList) {
      ...
      if (!selectNode) return store.setCurrentNodeKey();
      store.setCurrentNode(selectNode);
      if (this.lazy) return;
